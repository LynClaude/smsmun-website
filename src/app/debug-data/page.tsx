...
export default function DebugDataPage() {
  ...
  const debugData = async () => {
    ...
    // 3. 测试荣誉顾问表
    try {
      const { data, error } = await supabase.from('honor_advisors').select('*').limit(10)
    ...
    try {
      ...
      results.statusCounts = { success: false, error: err.message }
    }
      ...
      <div className="min-h-screen bg-gray-50 pt-24 pb-12">
        <div className="container mx-auto px-4">
          <div className="text-center">
            ...
            <p className="text-gray-600">检查数据中...</p>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-24 pb-12">
      <div className="container mx-auto px-4 max-w-6xl">
        <h1 className="text-3xl font-bold text-center mb-8">数据调试页面</h1>
        ...
        <div className="space-y-6">
          ...
          <div className={`p-6 rounded-lg border ${
            ...
            <p className="text-sm">总记录数: {results.honorAdvisors?.count || 0}</p>
            {results.honorAdvisors?.data?.length > 0 && (
              <div className="mt-4">
                <h3 className="font-medium mb-2">数据预览:</h3>
                <pre className="text-xs bg-white p-3 rounded border overflow-auto max-h-40">
                  {JSON.stringify(results.honorAdvisors.data.slice(0, 3), null, 2)}
                </pre>
              </div>
            )}
          </div>
          ...
          {/* 状态统计 */}
          <div className={`p-6 rounded-lg border ${
            results.statusCounts?.success ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
          }`}>
            <h2 className="text-xl font-semibold mb-4">
              状态统计: {results.statusCounts?.success ? '✅ 成功' : '❌ 失败'}
            </h2>
            <p className="text-sm mb-2">错误: {results.statusCounts?.error || '无'}</p>
            {results.statusCounts?.data?.length > 0 && (
              <div className="mt-4">
                <h3 className="font-medium mb-2">状态分布:</h3>
                <pre className="text-xs bg-white p-3 rounded border overflow-auto">
                  {JSON.stringify(results.statusCounts.data, null, 2)}
                </pre>
              </div>
            )}
          </div>
